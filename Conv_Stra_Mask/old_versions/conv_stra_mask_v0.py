...
Input:
    ...
    XLAT_data   1/2-D Latitude coordinates (e.g., XLAT from WRF model output)
    
    XLONG_data  1/2-D Longitude coordinates (e.g., XLONG from WRF model output)
...
def conv_stra_sep(dbz, lat2d, lon2d, grid_res, coor_type, method, CoreThresh=40.0, tune_thresh=42.43, bg_diff=10, fill_dbz=0.0):
    ...
    if coor_type == 'C':
        bkgnd_lin = ndi.uniform_filter(zlin, size=20/(grid_res*100), mode='nearest') # Rough estimated distance!
    elif coor_type == 'L':
