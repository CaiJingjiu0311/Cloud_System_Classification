...
Purpose:
    This program takes a 2D gridded reflectivity field and designates each point convective or stratiform based upon either the [ Steiner, Houze and Yuter (SHY 1995 - JAMS) ] or [ Yuter and Houze (YH 1998 - QJMS; YH 1997 - JAM)  ] algorithm. 
    The module handles either the Cartesian-gridded data (Lat-Lon coordinates) or the Lambert-conformal-gridded data (constant distance grid) depending on the coordinates of input dataset. The observational radara data are usually Cartesian-gridded; while the Lambert-conformal coordinates are common among models output (e.g. WRF). 

Calling sequence:
    conv_stra_sep
    
Run: 
    CS_mask, Conv_Cent, Bkgnd_REFL = conv_stra_sep( dbz_data, XLAT_data, XLONG_data, coor_type, grid_res )

Input:
    
    dbz_data    2-D radar reflectivity data (dBZ)
    
    XLAT_data   1/2-D Latitude coordinates (e.g., XLAT from WRF output)
    
    XLONG_data  1/2-D Longitude coordinates (e.g., XLONG from WRF output)
                NOTE: The XLAT/XLONG are only used when the input data is Cartesian-gridded (in Lat-Lon coordinates)
    
    coor_type   Types of input data coordinates
                NOTE: 'C' - Cartesian-gridded data
                      'L' - Lambert-conformal-gridded data)
    
    grid_res    Resolution of the grid 
                NOTE: Unit of 'degree' if the Cartesian coordinate is specified
                      Unit of 'km' for Lambert-conformal coordinate 
...
Optional Keywords:
    ...
    CoreThresh  Intensity threshold to identify convective centers. 
                Any grid point of dBZ over this value is automatically labeled as a convective center (YH98 uses 46).
                Default value: 40.0
    ...
    fill_ndbz   Filling this dBZ value for the missing (NaN) data points.
                The user may adjust the input data if the missing data points have already been set to certain value.
                Default value: -30.0

    stra_thresh The minimum dBZ threshold for the Stratiform identification, i.e. any grid point of dBZ over this value 
                is considered as Stratiform echo if it has not been identified as Convective echo.
                Default value: 15.0
...
Output:
    ...
    CS_mask     2-D array with the Convective-Stratiform Masks produced by this algorithm based on Surrounding area 
                ...
                = 0       stratiform (where reflectivity >= 15 dBZ)
...
Author and history:
    ...
    Feb 2022    Updated from Ver.0 to Ver.1 by Hungjui Yu. 
                Bugs fixed. Parameters adjusted. More user-defined arguments added. 
                Add ability to handle data if not in appropriate forms.
...
def mask_conv_area_C(bkgnd_range_id, outarray, latarray, lonarray, grid_res, dist_thresh, assign_value):
    ...
    box_size = np.ceil((5/(grid_res*100))) # Roughly estimated distance!
...
def conv_stra_sep(dbz, lat, lon, coor_type, grid_res, method='SHY', CoreThresh=40.0, tune_thresh=42.43, bg_diff=10, fill_ndbz=-30.0, stra_thresh=15.0):
    ...
    ## Replace the NaN grid points in the  data to the specified value:
    ...
    ## Set the Conv./Stra. Masks anywhere the dBZ is greater than the specified Stratiform threshold (by default 15.0 as in YH97, 98).
    ## Set it to 0 as Stratiform echo:
    cs[(dbz >= stra_thresh)] = 0
    ...
    if coor_type == 'C':
        bkgnd_lin = ndi.uniform_filter(zlin, size=20/(grid_res*100), mode='nearest') # Roughly estimated distance!
    elif coor_type == 'L':
